...
//        self.quad = quad ?? EditScanViewController.defaultQuad(forImage: image)
        let fixedImage = image.withFixedOrientation()
            ...
            self.quad = quad ?? EditScanViewController.defaultQuad(forImage: fixedImage)
...
//        quadView.drawQuadrilateral(quad: transformedQuad, animated: false)
        ...
        let imageSize = image.size
            ...
            // 2. The rect where the image is actually drawn inside the UIImageView
            let imageFrame = AVMakeRect(aspectRatio: imageSize, insideRect: imageView.bounds)
            
            // 3. Scale transform from image coordinates to displayed rect coordinates
            let scaleTransform = CGAffineTransform.scaleTransform(forSize: imageSize,
                                                                  aspectFillInSize: imageFrame.size)
            
            // 4. Translate transform to account for the offset (since imageFrame may not start at (0,0))
            let translationTransform = CGAffineTransform(translationX: imageFrame.origin.x,
                                                         y: imageFrame.origin.y)
            
            // 5. Apply both transforms
