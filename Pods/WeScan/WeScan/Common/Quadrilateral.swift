...
extension Quadrilateral {
    ...
    func toCartesian(withHeight height: CGFloat, yOffset: CGFloat = 0) -> Quadrilateral {
...
//        return Quadrilateral(topLeft: topLeft, topRight: topRight, bottomRight: bottomRight, bottomLeft: bottomLeft)
        ...
        let transform = CGAffineTransform(translationX: 0, y: -CGFloat(yOffset))
            
            let topLeft = self.topLeft.cartesian(withHeight: height).applying(transform)
            let topRight = self.topRight.cartesian(withHeight: height).applying(transform)
            let bottomRight = self.bottomRight.cartesian(withHeight: height).applying(transform)
            let bottomLeft = self.bottomLeft.cartesian(withHeight: height).applying(transform)
...
extension Quadrilateral {
    func toUIKitCoordinates(from imageSize: CGSize, to viewSize: CGSize, orientation: UIImage.Orientation) -> Quadrilateral {
        var transformed = self

        // 1. Scale quad from image space → view space
        transformed = transformed.scale(imageSize, viewSize)

        // 2. Flip Y-axis (Vision → UIKit coordinates)
        transformed = transformed.applying(CGAffineTransform(scaleX: 1, y: -1))
        transformed = transformed.applying(CGAffineTransform(translationX: 0, y: viewSize.height))

        // 3. Handle rotation based on image orientation
        switch orientation {
        case .right:
            transformed = transformed.applying(CGAffineTransform(rotationAngle: .pi/2))
            transformed = transformed.applying(CGAffineTransform(translationX: viewSize.width, y: 0))
        case .left:
            transformed = transformed.applying(CGAffineTransform(rotationAngle: -.pi/2))
            transformed = transformed.applying(CGAffineTransform(translationX: 0, y: viewSize.height))
        case .down:
            transformed = transformed.applying(CGAffineTransform(rotationAngle: .pi))
            transformed = transformed.applying(CGAffineTransform(translationX: viewSize.width, y: viewSize.height))
        default:
            break
        }
