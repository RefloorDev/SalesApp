...
//        }
    public required init(image: UIImage? = nil, delegate: ImageScannerControllerDelegate? = nil) {
        ...
        if #available(iOS 13.0, *) {
            ...
            navigationBar.backgroundColor = UIColor(red: 135, green: 135, blue: 135, alpha: 0.5)
        } else {
            navigationBar.tintColor = .black
            navigationBar.backgroundColor =  UIColor(red: 135, green: 135, blue: 135, alpha: 0.5)
        }
...
extension ImageScannerController {
    ...
    private func updateCameraOrientation() {
        // Find the camera view controller
        if let cameraVC = self.viewControllers.first(where: { $0 is CameraScannerViewController }) {
            let mirror = Mirror(reflecting: cameraVC)
            for child in mirror.children {
                if child.label == "captureSession", let captureSession = child.value as? AVCaptureSession {
                    // Set orientation for all video connections
                ...
                if child.label == "previewLayer", let previewLayer = child.value as? AVCaptureVideoPreviewLayer {
...
//class LandscapeImageScannerController: ImageScannerController {
//    
//    override open func viewDidLoad() {
//        super.viewDidLoad()
//        setupLandscapeCamera()
//    }
//    
...
//    }
//    
//    private func setupLandscapeCamera() {
//        // Find and configure the camera session for landscape
//        if let scannerVC = self.viewControllers.first as? ScannerViewController {
//            let scannerMirror = Mirror(reflecting: scannerVC)
//            
//            for child in scannerMirror.children {
//                if child.label == "captureSession", let captureSession = child.value as? AVCaptureSession {
//                    for connection in captureSession.connections {
//                        if connection.isVideoOrientationSupported {
//                            connection.videoOrientation = .landscapeRight
