...
class OrderStatusViewController: UIViewController,DropDownDelegate,UITextViewDelegate
{
    ...
    {
        logScreenEvent(screen: ScreenNames.appointmentResult) {
            var networkMessage = ""
            ...
            speedTest.testUploadSpeed { speed in
                ...
                let parameters:[String:Any] = ["appointment_id": AppointmentData().appointment_id ?? 0,"screen_name":ScreenNames.appointmentResult,"screen_entry_date":Date().getSyncDateAsString(),"network_strength":networkMessage,"timezone":timeZone,"create_date": Date().getSyncDateAsString()]
        ...
        group.notify(queue: .main) {
            ...
            case .addedRooms:
                ...
                speedTest.testUploadSpeed { speed in
                    ...
                    networkMessage = String(format: "%.2f", speed)
                    networkMessage += "Mbps"
                    let requestParams:[String:Any] = ["token" :UserData.init().token ?? "","appointment_id":appoint_id,"network_strength": networkMessage,"create_date": Date().getSyncDateAsString()]
                    HttpClientManager.SharedHM.initiateSync_i360_APi(parameter: requestParams) { success, message in
                    }
                }
