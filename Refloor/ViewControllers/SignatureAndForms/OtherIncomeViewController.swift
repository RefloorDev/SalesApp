...
import GoogleMaps
import GooglePlaces
class OtherIncomeViewController: UIViewController,DropDownDelegate,UITextFieldDelegate {
    static public func initialization() -> OtherIncomeViewController? {
        return UIStoryboard(name:"Main", bundle: nil).instantiateViewController(withIdentifier: "OtherIncomeViewController") as? OtherIncomeViewController
    }
    ...
    override func viewDidLoad() {
        ...
        {
            paymentPlanDropdownString = "NO LOAN APPLIED"
        ...
        self.DropDownDidSelectedAction(0, "SOCIAL SECURITY", sourceOfIncomeChangeTag)
    ...
    func getParameterForRefloorLocalAppsteamOdooServer() -> [String:Any]
    {
        return  [
            ...
            "owners": self.ownersType.text ?? "",
            "owners_if_different":self.owners.text ?? "",
            ...
            "source_of_other_income": self.sourceOfIncome.text ?? "",
            ...
            "address_relationship": self.address.text ?? "",
            ...
            "phone_number_relationhip": self.phoneNumber.text ?? "",
            "property_details": self.mortgage.text ?? "",
            "lender_name": "\(self.lenderFirstName.text ?? "") \(self.lenderMiddleName.text ?? "") \(self.landerLastName.text ?? "")",
            "lender_address": self.landerAddress.text ?? "",
            "lender_address_street": "",
            "lender_address_street2": "",
            "lender_address_city": "",
            "lender_address_state": "",
            "lender_address_zip": "",
            "lender_phone": self.landerPhoneNumber.text ?? "",
            ...
            "date_aquired": self.dateAcquired.text ?? "",
            ...
            "other_obligations": Double(self.otherObligations.text ?? "") ?? 0,
            "total_monthly_payments": Double(self.totalMonthlyPayments.text ?? "") ?? 0,
            "checking_account_no": self.checkingAccountNumber.text ?? "",
            "name_of_bank": self.nameOfBank.text ?? "",
            "bank_phone_number": self.otherPhoneNumber.text ?? "",
            "insurance_company": self.InsuranceCompany.text ?? "",
            "agent": self.agent.text ?? "",
            "insurance_phone_no": self.insurenceCompanyNumber.text ?? "",
            ...
            "applicant_signature_date": Date().DateFromStringForServer(),
            "co_applicant_signature_date": Date().DateFromStringForServer(),
            ...
            "co_applicant_email":self.coApplicantData?.CoapplicantEmail ?? "","co_applicant_skip":isCoAppSkiped,"co_applicant_phone":self.coApplicantData?.cellPhone ?? "","co_applicant_secondary_phone":self.coApplicantData?.homePhone ?? ""
    ...
    else if(addressToBeImproveType.text!.removeUnvantedcharactoes() == "DIFFERENT")
    {
...
extension OtherIncomeViewController: GMSAutocompleteViewControllerDelegate {
