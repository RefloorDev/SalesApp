...
class ApplicantFormViewController: UIViewController,DropDownDelegate,AddressSelectPlaceEntryDelegate,UITextFieldDelegate,GMSMapViewDelegate{
    ...
    static public func initialization() -> ApplicantFormViewController? {
        return UIStoryboard(name:"Main", bundle: nil).instantiateViewController(withIdentifier: "ApplicantFormViewController") as? ApplicantFormViewController
    }
    ...
    var wishShareStatus:HispanicStatusApplication = .Share
    var hispanicstatus:HispanicStatusApplication = .None
    ...
    override func viewDidLoad() {
        ...
        if let customer = AppDelegate.appoinmentslData
        {
            ...
            self.cellPhone.text = customer.phone
...
//        }
    ...
    @IBAction func nextButtonAction(_ sender: Any) {
        ...
        let data = ApplicationData.init(applicantFirstName: self.firstName.text ?? "", applicantMiddleName: self.middlename.text ?? "", applicantLastName: self.lastName.text ?? "", driversLicense: self.drivingLisenceID.text ?? "", driversLicenseExpDate: self.lisenceExpDate.text ?? "",driversLicenseIssueDate: self.lisenceExpDate.text ?? "", dateOfBirth: self.dateofbirth.text ?? "", socialSecurityNumber: self.socialSecurityNo.text ?? "", addressOfApplicant: self.address.text ?? "", addressOfApplicantStreet: "", addressOfApplicantStreet2: "", addressOfApplicantCity: self.city.text ?? "", addressOfApplicantState: self.stateZipCode.text ?? "", addressOfApplicantZip: self.zipCode.text ?? "", previousAddressOfApplicant: self.exAddress.text ?? "", previousAddressOfApplicantStreet: "", previousAddressOfApplicantStreet2: "", previousAddressOfApplicantCity: self.exCity.text ?? "", previousAddressOfApplicantState: self.exState.text ?? "", previousAddressOfApplicantZip: self.exZipCode.text ?? "", cellPhone: self.cellPhone.text ?? "", homePhone: self.homePhone.text ?? "", howLong: self.howLong.text ?? "", previousAddressHowLong: self.exHowLong.text ?? "", presentEmployer: self.presentEmployer.text ?? "", yearsOnJob: self.yearonJob.text ?? "", occupation: self.occupation.text ?? "", presentEmployersAddress: self.empAddress.text ?? "", presentEmployersAddressStreet: "", presentEmployersAddressStreet2: "", presentEmployersAddressCity: "", presentEmployersAddressState: "", presentEmployersAddressZip: "", earningsFromEmployment: self.earnings.text ?? "", supervisorOrDepartment: self.department.text ?? "", employersPhoneNumber: self.employementPhone.text ?? "", previousEmployersAddress: "", previousEmployersAddressStreet: "", previousEmployersAddressStreet2: "", previousEmployersAddressCity: "", previousEmployersAddressState: "", previousEmployersAddressZip: "", earningsPerMonth: "", yearsOnJobPreviousEmployer: "", occupationPreviousEmployer: "", previousEmployersPhoneNumber: "", ethnicity: ethencity, race: (wishShareStatus == .Share) ? (self.race.text ?? ""): ethencity, sex: sex,  maritalStatus: self.martialStatus.text ?? "",applicantEmail: self.emailAddress.text ?? "", type_of_credit_requested: creditRequest[selectedReq], otherRace: self.raceOther.text ?? "")
        ...
        if(AppDelegate.appoinmentslData.co_applicant_skipped == 1)
        {
            ...
            let applicant = OtherIncomeViewController.initialization()!
            ...
            let applicant = CoApplicantFormViewController.initialization()!
            applicant.downOrFinal = self.downOrFinal
            applicant.totalAmount = self.totalAmount
            applicant.paymentPlan = self.paymentPlan
            applicant.paymentPlanValue = self.paymentPlanValue
    ...
    @IBAction func lisenceExpDatePickAction(_ sender: UITextField) {
        ...
        //              datePickerMYOnly.backgroundColor =  UIColor.init(red: 40/255, green: 59/255, blue: 79/255, alpha: 1)
        //              datePickerMYOnly.tintColor = UIColor.white
        //              datePickerMYOnly.setValue(UIColor.white, forKeyPath: "textColor")
        //              //ToolBar
        //              let toolbar = UIToolbar();
        //              toolbar.barStyle = .blackTranslucent
        //              toolbar.sizeToFit()
        ...
        //              sender.inputAccessoryView = toolbar
        //              sender.inputView?.backgroundColor = UIColor.init(red: 40/255, green: 59/255, blue: 79/255, alpha: 1)
        //              sender.inputView = datePickerMYOnly
        //
        //                    self.datePickerMYOnly.onDateSelected = { (month: Int, year: Int) in
        //                        let string = String(format: "%02d/%d", month, year)
        //                        self.lisenceExpDate.text = string
...
extension ApplicantFormViewController:UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout
{
...
extension ApplicantFormViewController: GMSAutocompleteViewControllerDelegate {
...
enum HispanicStatusApplication
{
    case Hispanic
    case NotHispanic
    case DoNotShare
