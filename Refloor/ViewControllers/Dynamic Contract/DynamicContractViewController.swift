...
class DynamicContractViewController: UIViewController,PDFDocumentDelegate,UITextFieldDelegate, ContractCommentProtocol{
    ...
    {
        logScreenEvent(screen: ScreenNames.contractDocumnet) {
            var networkMessage = ""
            ...
            speedTest.testUploadSpeed { speed in
                ...
                let parameters:[String:Any] = ["appointment_id": AppointmentData().appointment_id ?? 0,"screen_name":ScreenNames.contractDocumnet,"screen_entry_date":Date().getSyncDateAsString(),"network_strength":networkMessage,"timezone":timeZone,"create_date": Date().getSyncDateAsString()]
...
//            installer.name = name
    ...
    func checkForInstallerOrNot(customerAndRoomData:[String:Any],appointmentId:Int)
    {
        ...
        if HttpClientManager.SharedHM.connectedToNetwork()
        {
            ...
            if isCardVerified == true
            {
                ...
                self.additionalComments(message: "Order details updated successfully", customerAndRoomData: customerAndRoomData, iscustomerAndRoomSuccess: iscustomerAndRoomSuccess, isNetwork: isNetwork, networkMessage: "")
            }
            else
            {
                DispatchQueue.main.async
    ...
    func proceedWithNetworkStrength(networkMessage:String,customerAndRoomData:[String:Any],appointmentId:Int,isNetwork:Bool)
    {
        DispatchQueue.main.async {
        HttpClientManager.SharedHM.showhideHUD(viewtype: .SHOW , title: "Creating Sale Order")
    ...
    func additionalComments(message:String,customerAndRoomData:[String:Any],iscustomerAndRoomSuccess:Bool,isNetwork:Bool,networkMessage:String)
    {
        DispatchQueue.main.async {
        HttpClientManager.SharedHM.showhideHUD(viewtype: .SHOW , title: "Creating Sale Order")
...
//                networkMessage = String(speed)
                self.additionalCommentsApiCall(message: message, customerAndRoomData: customerAndRoomData, iscustomerAndRoomSuccess: iscustomerAndRoomSuccess, isNetwork: isNetwork, networkMessage: networkMessage)
           // }
        }
    ...
    func additionalCommentsApiCall(message:String,customerAndRoomData:[String:Any],iscustomerAndRoomSuccess:Bool,isNetwork:Bool,networkMessage:String)
    {
        DispatchQueue.main.async {
        HttpClientManager.SharedHM.showhideHUD(viewtype: .SHOW , title: "Creating Sale Order")
            ...
            let appoint_id = AppointmentData().appointment_id ?? 0
            let recison = UserDefaults.standard.value(forKey: "Recision_Date") as! String
            parametersAdditionalComments = ["token": UserData.init().token ?? "" ,"appointment_id":appoint_id,"flexible_installation":self.FlexInstall ? 1: 0,"send_physical_document":self.sendPhysicalDocument ? 1 : 0,"additional_comments":self.comments,"recision_date":recison,"network_strength":networkMessage,"both_parties_present":self.isBothParties,"create_date": Date().getSyncDateAsString()]
            ...
            HttpClientManager.SharedHM.additionalCommentsAPi(parameter: parametersAdditionalComments) { success, usermessage in
                ...
                else if success == "Failed"
                {
                    let yes = UIAlertAction(title: "Retry", style:.default) { (_) in
                        ...
                        self.additionalComments(message: message, customerAndRoomData: customerAndRoomData, iscustomerAndRoomSuccess: iscustomerAndRoomSuccess, isNetwork: isNetwork, networkMessage: "")
    ...
    func whetheToProceedToInstaller(customerAndRoomData:[String:Any],iscustomerAndRoomSuccess:Bool,isNetwork:Bool,appointmentId:Int)
    {
        ...
        if appointment?.enableDestinationSelection == 1
        {
            ...
            {
                let recison = UserDefaults.standard.value(forKey: "Recision_Date") as! String
                parametersAdditionalComments = ["appointment_id":appoint_id,"contract_plumbing_option_1":contract_plumbing_option_1, "contract_plumbing_option_2" : contract_plumbing_option_2,"recision_date" : recison,"send_physical_document": self.sendPhysicalDocument ? 1 : 0,"additional_comments":self.comments, "flexible_installation": self.FlexInstall ? 1: 0]
            }
            let installer = DestinationMotivationViewController.initialization()!
            installer.name = name
