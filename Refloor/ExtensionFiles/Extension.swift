...
//            print(RealmError.initialisationFailed.rawValue)
    ...
    func getDebugAptDataToUpload() -> Results<rf_Debug_Appointment_Log>!
    {
        var appointmentData :Results<rf_Debug_Appointment_Log>!
        do
        {
            let realm = try Realm()
            let appointment = realm.objects(rf_Debug_Appointment_Log.self)
            appointmentData = appointment
            return appointmentData
        }
            catch{
