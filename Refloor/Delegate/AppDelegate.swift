...
//    public static var SCANBOT_LICENSE_KEY =
//        "nInZv1NB/hmS3rNaarAA9iPSKgdkSA" + "ojW88qRWsVMe6C8OokNoszskE7GOKl"
//        + "RB8568Qh5WNdv/NceRlFl/vknJJ3PA" + "21R+lwqG/7gy1HT8ZBPYfpq4ZXCoVr"
//        + "IeUMGDPxk+Xjn+iku751Hqcl+Ls/v7" + "if2jqEEjOckOg/oVZ40oGUriJ8THOn"
//        + "Mu4SJqXjQRYXTVhE3Y/SUQg3NzcC8M" + "QOk3PG3K0/dO58gGZjHiDVFDhAb/M6"
//        + "5vQrOCag8A3vW53GtllJC6zSKBBiIR" + "4MZxUHe7AhcrXZ15n9v2uj/KWJqv4J"
//        + "iRKx7HSlJcnmCEky+uy4SPtCb6KXYr" + "njjJ9Qa8hHpg==\nU2NhbmJvdFNESw"
//        + "pjb20ucmVmbG9vci5zYWxlc2FwcC5v" + "Y3IKMTc3ODg4OTU5OQo4Mzg4NjA3Cj"
//        + "E5\n"  //  testing_key_ one week trail
    
    
    public static var SCANBOT_LICENSE_KEY =
        "dCXwZNxhhUZREbipckAu/9eysCN1If" + "nK6vuT4+GfEcthqZ68BVnN4HHtPgsB"
        + "8OfN8vpVfXfxhPXschGxyU/ilCct2i" + "G2wXLptJxIGDMDdEvracsvLfeh9LjH"
        + "jwD3DZ9jQ0xD/w+lIv79dwDS4E4e19" + "aR410RIyvxU0NEDvsR/oNi02YoO2su"
        + "Murc4peKN/pl+ETYcw1xxvRz/oapLK" + "xIkp64Hs0PQ0z9VouuqE56PywqJmsN"
        + "8RyTpqv5x88q5LYfPy2fe9IpJKnHy9" + "aJwUEa/jF3kW/cYvneRkE4qEGGwcVq"
        + "AqYQB16IYpG1JcLX/0CtaLL5trTmwA" + "uQIgpOOn2hOA==\nU2NhbmJvdFNESw"
        + "pjb20ucmVmbG9vci5zYWxlc2FwcHxj" + "b20ucmVmbG9vci5zYWxlc2FwcC5vY3"
        + "IKMTgxMjQxMjc5OQo4Mzg4NjA3CjE=" + "\n"
    ...
    func application(
        ...
        Scanbot.setLicense(AppDelegate.SCANBOT_LICENSE_KEY)

        let config = Realm.Configuration(
            // Set the new schema version. This must be greater than the previously used
            // version (if you've never set a schema version before, the version is 0).
            schemaVersion: 40,  //7, // production 12
        ...
        // Ensure Realm files are accessible in background even if device is locked
        if let realmURL = config.fileURL {
            let folderURL = realmURL.deletingLastPathComponent()
            try? FileManager.default.setAttributes([FileAttributeKey.protectionKey: FileProtectionType.completeUntilFirstUserAuthentication], ofItemAtPath: folderURL.path)
            try? FileManager.default.setAttributes([FileAttributeKey.protectionKey: FileProtectionType.completeUntilFirstUserAuthentication], ofItemAtPath: realmURL.path)
        }
        ...
        AppDelegate.locationManager?.desiredAccuracy =
    ...
    func userNotificationCenter(
        ...
        let vc =
            sbMain.instantiateViewController(
                withIdentifier: "ViewLogListViewController"
            ) as! ViewLogListViewController
...
extension AppDelegate: CLLocationManagerDelegate {
    ...
    func saveScreenCompletionTimeToDb(
        appointmentId: Int,
        className: String,
        displayName: String,
        time: Date
        ...
        let realm = try! Realm()
        let completionTime = ScreenCompletion(
    ...
    func locationManager(
        ...
        if region is CLCircularRegion {
            ...
            self.saveScreenCompletionTimeToDb(
                appointmentId: Int(region.identifier) ?? 0,
            ...
            let entryTime = geoLocationTimeForAppointments(
                appointmentId: Int(region.identifier) ?? 0
            )
            ...
            do {
                ...
                try realm.write {
                    ...
                    let geoLocationValues: [String: Any] = [
                        "appointmentId": appointmentId!, "entryTime": entryTime,
                        "exitTime": exitTime, "syncStatus": false,
                    ]
                    realm.create(
                        rf_GeoLocationData.self,
                        value: geoLocationValues,
                        update: .all
                    )
                    let token = UserData.init().token!
                    parameters = [
                        "appointment_id": Int(region.identifier) ?? 0,
                        "arrival_date": entryTime, "departure_date": exitTime,
                        "token": token,
                    ]
            ...
            geoLocationApiCall(parameter: parameters) { success in
                if success! {
                    ...
                    let geoLocationValues: [String: Any] = [
                        "appointmentId": appointmentId!, "entryTime": entryTime,
                        "exitTime": exitTime, "syncStatus": true,
                    ]
                    self.updatingGeolocationSyncValues(
    ...
    func getAppointmentData(appointmentId: Int) -> rf_master_appointment! {
        ...
        do {
            let realm = try Realm()
            if let appointment = realm.objects(rf_master_appointment.self)
                .filter("id == %d", appointmentId).first
            {
                appointmentData = appointment
            } else if let appointmentReq = realm.objects(
                rf_Completed_Appointment_Request.self
            ).filter(
                "reqest_title == %d AND appointment_id == %d",
                RequestTitle.CustomerAndRoom.rawValue,
                appointmentId
            ).first {
                let customerFullDict =
                    appointmentReq.request_parameter?.dictionaryValue() ?? [:]
                ...
                let customerDict =
                    customerFullDict["customer"] as? [String: Any] ?? [:]
                appointmentData = rf_master_appointment(
                    appointmentObj: customerDict
                )
            }
        } catch {
    ...
    func saveLogDetailsForAppointment(
        appointmentId: Int,
        logMessage: String,
        time: String,
        errorMessage: String = "",
        ...
        do {
            ...
            try realm.write {
                ...
                if appointmentLogs.count == 0 {
                    ...
                    let dict: [String: Any] = [
                        "appointment_id": appointmentId,
                        "sync_message": logList, "appBaseUrl": BASE_URL,
                        "paymentStatus": payment_status,
                        "paymentMessage": payment_message,
                    ]
                    realm.create(
                        rf_Appointment_Logs.self,
                        value: dict,
                        update: .all
                    )
                } else {
                    if let appointmentLogsAlreadyExist = appointmentLogs.first {
                        let logArrayList = appointmentLogsAlreadyExist
                            .sync_message
                        logArrayList.append(
                            rf_Appointment_Log_Data(
                                message: logMessage,
                        ...
                        if payment_status == "" {
                            let pay_message = appointmentLogsAlreadyExist
                                .paymentMessage

                            let pay_status = appointmentLogsAlreadyExist
                                .paymentStatus
                            dict = [
                                "appointment_id": appointmentId,
                                "sync_message": logArrayList,
                                "appBaseUrl": BASE_URL,
                                "paymentStatus": pay_status ?? "",
                                "paymentMessage": pay_message ?? "",
                            ]
                        } else {
                            dict = [
                                "appointment_id": appointmentId,
        ...
        for resultsData in geoLocationData {
            if resultsData.entryTime != "" || resultsData.exitTime != "" {
                ...
                let parameter: [String: Any] = [
                    "appointment_id": resultsData.appointmentId,
                    "arrival_date": resultsData.entryTime!,
                    "departure_date": resultsData.exitTime!, "token": token,
                ]
    ...
    func geoLocationApiCall(
        ...
        if HttpClientManager.SharedHM.connectedToNetwork() {
            ...
            HttpClientManager.SharedHM.geoLocationTimeSubmitAPi(
    ...
    func geoLocationTimeForAppointments(appointmentId: Int) -> String {
        ...
        do {
            let realm = try Realm()
            if let appointment = realm.objects(rf_GeoLocationData.self).filter(
                "appointmentId == %d",
                (appointmentId)
            ).first {
    ...
    func geoLocationDataForSync() -> RealmSwift.Results<rf_GeoLocationData> {
        ...
        do {
            let realm = try Realm()
            geoLocationData = realm.objects(rf_GeoLocationData.self).filter(
                "syncStatus == %@",
                (false)
            )
    ...
    func locationManager(
        ...
        if region is CLCircularRegion {
            ...
            var parameters: [String: Any] = [:]
            let entryTime = geoLocationTimeForAppointments(
                appointmentId: Int(region.identifier)!
            )
            let appointment = self.getAppointmentData(
                appointmentId: Int(region.identifier) ?? 0
            )
            ...
            if entryTime == "" {
                ...
                do {
                    ...
                    try realm.write {
                        ...
                        parameters = [
                            "appointment_id": Int(region.identifier) ?? 0,
                            "arrival_date": entryTime, "departure_date": "",
                            "token": token,
                        ]
                    }
                } catch {
    ...
    func locationManager(
        ...
        for region in monitoredRegions {
                ...
                circularRegion.contains(location.coordinate)
            {
                ...
                var parameters: [String: Any] = [:]
                var entryTime = geoLocationTimeForAppointments(
                    appointmentId: Int(region.identifier)!
                )
